...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-[240px] bg-bg-surface border-r border-border z-50 flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="h-14 flex items-center px-5 border-b border-border">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded bg-accent-primary flex items-center justify-center">
              <span className="text-white text-[13px] font-bold">{companyName[0]}</span>
            </div>
            <span className="text-[15px] font-semibold text-txt-primary tracking-tight">{companyName}</span>
          </div>
        </div>
        ...
        <nav className="flex-1 py-2 px-3 overflow-y-auto">
          {nav.map(item => (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-md text-[13px] font-medium transition-colors mb-0.5 ${
                  isActive
        ...
        <div className="px-5 py-3 border-t border-border">
          <div className="text-[11px] text-txt-tertiary">v1.0.0 · {companyName}</div>
        </div>
      </aside>
    </>
  )
